...
- сеттер контента заменяет содержимое модального окна на переданный элемент.

#### Класс BasketView
Класс управляет отображением корзины интернет-магазина. Отвечает за вывод списка товаров, подсчёт общей суммы и управление состоянием кнопки оформления заказа. Поддерживает вывод сообщения при пустой корзине.

Поля класса:
- list: HTMLElement — контейнер для списка товаров;
- _total: HTMLElement — элемент с итоговой стоимостью заказа;
- button: HTMLButtonElement — кнопка «Оформить»;
- events: IEvents — брокер событий для связи с презентером.

Методы:
- setButtonState(items: string[]): void — изменяет состояние кнопки в зависимости от того, пуста корзина или нет;
- сеттеры товаров и общей суммы заказа.
...
- _error: HTMLElement — элемент где выводятся ошибки;
- events: IEvents — брокер событий для взаимодействия с презентером.

Класс предоставляет следующие методы:
- сеттеры ошибки и валидности формы для изменения состояния кнопки.

#### Класс DeliveryFormView
Класс расширяет функциональность FormView для отображения и обработки формы выбора способа оплаты и адреса. Управляет выбором варианта оплаты, полем ввода адреса и переходом к следующему шагу оформления заказа.
...
Класс расширяет функциональность FormView для отображения и обработки формы с контактными данными пользователя. Управляет вводом информации и инициирует завершение оформления заказа.

Поля класса:
- inputs: NodeListOf<HTMLInputElement> — поля ввода email и телефона.

#### Класс SuccessView
Класс отображает финальный экран успешного оформления заказа.

Поля класса:
- _total: HTMLElement — элемент с итоговой суммой заказа.
- closeButton: HTMLButtonElement — кнопка возврата в каталог.

Методы:
- сеттер итоговый суммы заказа.
...
| Событие                   | Описание                                  |
| ------------------------- | ----------------------------------------- |
| `initialData:loaded`      | данные с API подгрузились                 |
...
| `delivery:changed`        | изменение полей формы доставки            |
| `contacts:changed`        | изменение полей формы контактов           |
...
| `order:ready`             | данные заказа готовы к отправке на сервер |
| `success:close`           | закрытие модалки с успешным заказом       |
